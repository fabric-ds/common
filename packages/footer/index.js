class FabricDocsFooter extends HTMLElement {
  constructor() {
    super();

    const fabricStylesTemplate = document.createElement('template');
    fabricStylesTemplate.innerHTML = `
      <style>:host { display: block; }</style>
      <link
          rel="stylesheet"
          type="text/css"
          href="https://assets.finn.no/pkg/@fabric-ds/css/v0/fabric.min.css"
      />
      <footer aria-orientation="horizontal" class="mt-32 text-12 text-gray-600">
        <hr class="mb-40" />
        <div id="docs-footer" class="grid grid-flow-col justify-end gap-24"></div>
      </footer>
    `;

    this.attachShadow({ mode: 'open' }).appendChild(
      fabricStylesTemplate.content
    );

    this.innerHTML = '';

    this.entries = JSON.parse(
      document.querySelector('[data-for="footer"]').textContent
    );

    this.render();
  }

  render() {
    const linkClass =
      'grid gap-6 grid-flow-col justify-start items-center  link link--block';

    this.shadowRoot.querySelector('#docs-footer').innerHTML += `
    ${
      this.entries.slack
        ? `<a class="grid gap-6 grid-flow-col justify-start items-center  link link--block" href="${this.entries.slack.href}" title="Slack channel" style="color: inherit;">
            <svg
              aria-hidden
              role="img"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 448 512"
              class="w-20 h-20"
            >
              <path
                fill="currentColor"
                d="M94.12 315.1c0 25.9-21.16 47.06-47.06 47.06S0 341 0 315.1c0-25.9 21.16-47.06 47.06-47.06h47.06v47.06zm23.72 0c0-25.9 21.16-47.06 47.06-47.06s47.06 21.16 47.06 47.06v117.84c0 25.9-21.16 47.06-47.06 47.06s-47.06-21.16-47.06-47.06V315.1zm47.06-188.98c-25.9 0-47.06-21.16-47.06-47.06S139 32 164.9 32s47.06 21.16 47.06 47.06v47.06H164.9zm0 23.72c25.9 0 47.06 21.16 47.06 47.06s-21.16 47.06-47.06 47.06H47.06C21.16 243.96 0 222.8 0 196.9s21.16-47.06 47.06-47.06H164.9zm188.98 47.06c0-25.9 21.16-47.06 47.06-47.06 25.9 0 47.06 21.16 47.06 47.06s-21.16 47.06-47.06 47.06h-47.06V196.9zm-23.72 0c0 25.9-21.16 47.06-47.06 47.06-25.9 0-47.06-21.16-47.06-47.06V79.06c0-25.9 21.16-47.06 47.06-47.06 25.9 0 47.06 21.16 47.06 47.06V196.9zM283.1 385.88c25.9 0 47.06 21.16 47.06 47.06 0 25.9-21.16 47.06-47.06 47.06-25.9 0-47.06-21.16-47.06-47.06v-47.06h47.06zm0-23.72c-25.9 0-47.06-21.16-47.06-47.06 0-25.9 21.16-47.06 47.06-47.06h117.84c25.9 0 47.06 21.16 47.06 47.06 0 25.9-21.16 47.06-47.06 47.06H283.1z"
              ></path>
            </svg>
            ${this.entries.slack.name}
          </a>`
        : ''
    }
    ${
      this.entries.github
        ? `<a class="${linkClass}" href="${this.entries.github.href}" title="Github repository" style="color: inherit;">
            <svg
              aria-hidden
              role="img"
              class="w-20 h-20"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 20"
            >
              <path
                fill="currentColor"
                d="M10 0a10 10 0 0 0-3.16 19.49c.5.1.68-.22.68-.48l-.01-1.7c-2.78.6-3.37-1.34-3.37-1.34-.46-1.16-1.11-1.47-1.11-1.47-.9-.62.07-.6.07-.6 1 .07 1.53 1.03 1.53 1.03.9 1.52 2.34 1.08 2.91.83.1-.65.35-1.09.63-1.34-2.22-.25-4.55-1.11-4.55-4.94 0-1.1.39-1.99 1.03-2.69a3.6 3.6 0 0 1 .1-2.64s.84-.27 2.75 1.02a9.58 9.58 0 0 1 5 0c1.91-1.3 2.75-1.02 2.75-1.02.55 1.37.2 2.4.1 2.64.64.7 1.03 1.6 1.03 2.69 0 3.84-2.34 4.68-4.57 4.93.36.31.68.92.68 1.85l-.01 2.75c0 .26.18.58.69.48A10 10 0 0 0 10 0"
              ></path>
            </svg>
            ${this.entries.github.name}
          </a>`
        : ''
    }
    `;
  }
}

if (!customElements.get('f-docs-footer')) {
  customElements.define('f-docs-footer', FabricDocsFooter);
}
